...
def main(): 
	...
	pv = 'na' #  parametric variable? pa, na, or pref 
	...
	out_dir = os.path.join(data_dir,'results_cue_'+pv)  	# directory for out files 
	out_str = 'glm_bycond'					# string for output files
	...
	for subject in subjects:
		...
		cmd = ('3dDeconvolve '		
			...
			'-stim_file 17 regs/'+pv+'alcohol_trial_cuec.1D -stim_label 17 '+pv+'_alcohol ' 
			#'-stim_file 18 regs/drugs_trial_cuec.1D -stim_label 18 drugs ' 
			'-stim_file 18 regs/'+pv+'food_trial_cuec.1D -stim_label 18 '+pv+'_food ' 
			'-stim_file 19 regs/'+pv+'neutral_trial_cuec.1D -stim_label 19 '+pv+'_neutral ' 
			'-num_glt 2 '					 # of contrasts
			'-glt_label 1 food-neutral -gltsym "SYM: +food -neutral" ' 
			'-glt_label 2 '+pv+'_alcfoodneutral -gltsym "SYM: +'+pv+'_alcohol +'+pv+'_food +'+pv+'_neutral" ' 
